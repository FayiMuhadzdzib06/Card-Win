...
function checkout($data)
{
    ...
    foreach ($_SESSION["cart"] as $product_id => $result) {
        ...
        $total_harga = $result * $barang["harga_barang"];
...
function editBarang($data)
{
    ...
    if (empty($foto)) {
        $query = mysqli_query($koneksi, "UPDATE barang SET nama_barang =  '$nama', jenis_barang =  '$jenis', harga_barang =  '$harga', stok_barang =  '$stok' WHERE id_barang = '$id'");
    } else {
        $query = mysqli_query($koneksi, "UPDATE barang SET nama_barang =  '$nama', jenis_barang =  '$jenis', harga_barang =  '$harga', stok_barang =  '$stok' foto = '$foto' WHERE id_barang = '$id'");
        move_uploaded_file($files, "C:/xampp/htdocs/Pemrograman_Web_FayiMuhadzdzib/images/" . $foto);
    }
